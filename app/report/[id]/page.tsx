...
function ReportMessage({ title, body }: { title: string; body: string }) {
  return (
    <ErrorCard className="text-center">
      <h1 className="text-[30px] font-bold text-[var(--color-ink)]">{title}</h1>
      <p className="mt-4 text-sm leading-relaxed text-[var(--color-slate)]">{body}</p>
      <Link
        href="/"
        className="mt-8 inline-flex h-11 items-center justify-center rounded-full bg-[var(--color-blue)] px-6 text-[14px] font-semibold text-white transition-colors duration-150 hover:brightness-95"
      >
        홈으로 돌아가기
      </Link>
    </ErrorCard>
...
function ReportContent({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    // endpoint — see docs/internal/security-notes.md.
