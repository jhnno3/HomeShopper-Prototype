...
/* Drift is transform-only (x/y translate) so it stays on the compositor —
   the earlier version also animated `scale`, which re-rasterizes the heavy
   blur every frame and was a big source of hero scroll jank on mobile.
  ...
  return (
    <>
      {BLOBS.map((blob, i) => (
        <motion.div
          key={i}
          className="absolute rounded-full opacity-45 blur-[72px] will-change-transform"
